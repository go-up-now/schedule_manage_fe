...
import { deleteClazz, updateAllStudyInIsTrueByStudent } from "../../../api/StudyIn.js";
import Modal from "../../../component/Modal.jsx";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faCircleExclamation, faMoneyCheck } from "@fortawesome/free-solid-svg-icons";
...
function PlanSubject() {
  ...
  const totalCredits = classes.reduce((total, clazz) => total + clazz.credits, 0);
    ...
    <div className="">
      ...
      <div className="flex justify-between">
        <div className="flex flex-col">
          <p>Tên học viên: <span className="font-medium ml-2">{student.lastName} {student.firstName}</span></p>
          <p>Mã học viên: <span className="font-medium ml-2">{student.code}</span></p>
          <p>Tổng số tín chỉ đăng ký: <span className="font-medium ml-2">{totalCredits}</span></p>
        </div>
        <div >
          <div className="flex flex-col justify-end">
            ...
            {classes.map((clazz) => (
              <div key={clazz.id} className="w-full flex justify-end">
                <p className="font-medium text-sm text-gray-400">+ {clazz.subject_code} - {clazz.price.toLocaleString()} VND</p> {/* Hiển thị giá */}
              </div>
            ))}
            <div className="border-t border-gray-400 w-full my-4 py-4 flex justify-end font-medium text-sm">
              Tổng: <p className="ml-2 text-red-500">{totalPrice.toLocaleString()} VND</p>
            </div>
      ...
      {selectedClazz && (
        ...
        >
          <div className="pt-2">
            <div className="flex h-28 items-center my-4">
              <div className="w-[150px] h-full flex items-center justify-center">
                <FontAwesomeIcon icon={faCircleExclamation} className="h-[60%] text-yellow-500" />
              </div>
              <div className="flex flex-col items-start justify-start">
                <p className="font-medium text-2xl">XÁC NHẬN HUỶ ĐĂNG KÝ</p>
                <p className="font-medium text-xl">{selectedClazz.subject_name}</p>
              </div>
            </div>
          </div>
          <div className="flex justify-between pt-4 border-t border-black">
            ...
            />
            <Button label={"Huỷ"} className="text-white justify-center bg-red-700 w-[150px] h-10 p-1" onClick={closeModal} />
          </div>
        </Modal>
      )}
