...
function InstructorManage() {
  const headers = ["Mã giảng viên", "Họ và tên", "Giới tính", "Ngày sinh", "Số điện thoại", ""];
  ...
  const formikInstructor = useFormik({
    initialValues: {
      ...
      gender: editInstructor ? (editInstructor.gender ? 'true' : 'false') : 'true',
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      const action = async () => {
        ...
        if (values.id === 0) {
          ...
          setLoading(true); // Bắt đầu loading
          try {
            const response = await updateInstructorAPI(values.id, formattedInstructor);
            if (response && response.data) {
              if (response.data.statusCode !== 200)
                toast.error(response.data.message)
              if (response.statusCode === 200) {
      ...
      values.id === 0 ? openConfirm(action, "Bạn có chắc muốn thêm giảng viên này?")
        : openConfirm(action, `Bạn có chắc muốn cập nhật giảng viên ${editInstructor?.code}?`)
    },
  });
          ...
          {selectedInstructor && (
            <Modal isOpen={true} onClose={closeModal} className="py-12">
              <h2 className="text-xl font-bold">
                {selectedInstructor.lastName + ' '}{selectedInstructor.firstName} - {selectedInstructor.code}
              </h2>
              <div>
